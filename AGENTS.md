...
To run the custom suite of unit test for Colony Gateway use the following sequence of commands that will install dependencies
...
- Increment (look at `CHANGELOG.md` for semver changes) the `version` value in `setup.py`, the try to find all the other places in the codebase where the version is referenced (which are many).
...
Colony Gateway is licensed under the [Apache License, Version 2.0](http://www.apache.org/licenses/).
